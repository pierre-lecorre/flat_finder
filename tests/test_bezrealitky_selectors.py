...
class TestBezrealitkyListingSelectors:
    ...
    def test_listing_cards_count_reasonable(self, soup):
        cards = soup.select("article.propertyCard")
        # Expect at least 5 and at most 50 cards per page
    ...
    def test_listing_urls_are_unique(self, soup):
        ...
        # Deduplicate (each card has multiple <a> tags pointing to the same URL)
    ...
    def test_price_additional_selector(self, soup):
        ...
        # Not every listing has additional costs, but at least some should
        # This is a soft check — just verify the selector does not crash
    ...
    def test_image_url_selector(self, soup):
        ...
        # Images are lazy-loaded; srcset or data-src may be used instead
        # We only check the selector runs without error
